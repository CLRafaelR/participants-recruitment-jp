...
//20180909
//下記リンクよりファイルをコピー
//https://raw.githubusercontent.com/grocio/participants-recruitment-jp/master/main.gs

//20180909
//ファイル修正・メール文言変更・国民の休日対応
...
function sendToCalendar(e){
  try{
    ...
    var experimenterMailAddress = "実験者のメールアドレス";
    var experimenterName = '実験者名'; //実験者名
    ...
    if (stime < opening || closing < etime) {
        var text = ParticipantName + "  様\n\n実験実施責任者の" + experimenterName +
          "です。\nこの度は【実験名・プロジェクト名】への応募ありがとうございました。\n" +
          '申し訳ありませんが、ご希望いただいた\n\n' + appo +
          '\n\nは実験実施可能時間（' + opening.getHours() + '時〜' + closing.getHours() + '時）外です。\n\n' +
          'お手数ですが、もう一度登録し直していただきますようお願いします。\n\n' + experimenterName;
        MailApp.sendEmail(ParticipantEmail, "【実験名・プロジェクト名】実験実施可能時間外です", text, {bcc: experimenterMailAddress});
    ...
    // --- カレンダーに既に登録された予定や予約と重複する時間に応募してきた場合 ---
        ...
        var text = ParticipantName + "  様\n\n実験実施責任者の" + experimenterName +
          "です。\nこの度は【実験名・プロジェクト名】実験への応募ありがとうございました。\n" +
          '申し訳ありませんが、ご希望いただいた\n\n' + appo +
          '\n\nにはすでに予約（予定）が入っているか、ほぼ同時で他の方が先に予約をされた可能性があり、実験を実施することができません。\n\n' +
          'お手数ですが、もう一度別の日時でご登録いただきますようお願いします。\n\n' + experimenterName;
        MailApp.sendEmail(ParticipantEmail, "【実験名・プロジェクト名】予約が重複しています", text, {bcc: experimenterMailAddress});
        sheet.getRange(num_row, 10).setValue('重複');
        sheet.getRange(num_row, 11).setValue(1);
        sheet.getRange(num_row, 12).setValue('N/A');
        sheet.getRange(num_row, 13).setValue('N/A');
    } else {
        var text = ParticipantName + "  様\n\n実験実施責任者の" + experimenterName + "です。\n" +
            "この度は【実験名・プロジェクト名】実験への応募ありがとうございました。\n予約の確認メールを自動で送信しております。\n\n" +
            appo + '\n\nにて' + ParticipantName + '様の仮予約を受け付けました（まだ確定はしていません）。\n\n' +
            '後日、予約完了のメールを送信いたします。\n\n' + 'もし日時の変更等がある場合は' + experimenterMailAddress +
            'までご連絡ください。\n今後ともよろしくお願いいたします。\n\n' + experimenterName;
        //予約確認メールを送信
        MailApp.sendEmail(ParticipantEmail, "【実験名・プロジェクト名】予約の確認", text, {bcc: experimenterMailAddress});
        //仮予約情報をカレンダーに作成
...
function updateCalendar(e){
  try{
    ...
    var experimenterName = "実験者名";
    var experimenterMailAddress = "実験者のメールアドレス";
    var experimenterPhone = "研究室の番号や実験者の携帯番号"; //研究室の番号や実験者の携帯番号
    var experimentRoom = "実験場所";
    var experimentLength = 60; //実験の長さ（単位は分）
    ...
    var holiday = calJa.getEventsForDay(stime).length;
    ...
    if(activeCell.getColumn()==10 && activeCell.getValue()==1 && sheet.getRange(activeCellRow,11)!=1){
      ...
      //メールの本文（土日に予約した場合）
      if(week == 0 || week == 6){
        var text = ParticipantName + "  様\n\nこの度は実験への応募ありがとうございました。\n" +
            hizuke + "からの【実験名・プロジェクト名】実験の予約が完了しましたのでメールいたします。\n" +
            "場所は" + experimentRoom + "です。休日は研究室のある建物の玄関の鍵がかかっており、外から入ることができません。実験開始5分前から玄関前で待機しておりますので、実験開始時間までにお越しください。\n" +
            "ご不明な点などありましたら、" + experimenterMailAddress +"までご連絡ください。\n" +
            "当日もよろしくお願いいたします。\n\n実験責任者 " + experimenterName +
            "（当日は他の者が実験担当する可能性もあります）\n" +
      ...
      MailApp.sendEmail(ParticipantEmail, "【実験名・プロジェクト名】実験予約完了いたしました", text, {bcc: experimenterMailAddress});
      //リマインダーのための設定をする
    ...
    // --- 以前実験に参加したことがあり参加を断るの場合 ---
        ...
        var text = ParticipantName + "  様\n\n実験実施責任者の" + experimenterName + "です。\nこの度は【実験名・プロジェクト名】への応募ありがとうございました。\n" +
            ...
            "ご不明な点などありましたら、" + experimenterMailAddress + "までご連絡ください。\n" +
            "またの機会での参加をお待ちしております。今後ともよろしくお願いします。\n\n" + experimenterName;
        //参加者にメールを送る
        MailApp.sendEmail(ParticipantEmail, "【実験名・プロジェクト名】既に同様の実験にご参加いただきました", text, {
    ...
    // --- もし実験期間外に応募してきた場合 ---
        ...
        var text = ParticipantName + "  様\n\n実験実施責任者の" + experimenterName + "です。\nこの度は【実験名・プロジェクト名】への応募ありがとうございました。\n" +
            ...
            "ご不明な点などありましたら、" + experimenterMailAddress + "までご連絡ください。\n\n" + experimenterName;
        //参加者にメールを送る
        MailApp.sendEmail(ParticipantEmail, "【実験名・プロジェクト名】実験実施期間外です", text, {
            bcc: experimenterMailAddress
        });
        sheet.getRange(activeCellRow, 11).setValue(1);
        sheet.getRange(activeCellRow, 12).setValue('N/A');
    ...
    // --- もし応募人数を超過した場合 ---
        ...
        var text = ParticipantName + "  様\n\n実験実施責任者の" + experimenterName + "です。\nこの度は【実験名・プロジェクト名】への応募ありがとうございました。\n" +
            ...
            "今後実施予定の、別の実験に再度ご応募いただけると幸いです。\n\n" +
            "ご不明な点などありましたら、" + experimenterMailAddress + "までご連絡ください。\n" +
            "今後ともよろしくお願いいたします。\n\n" + experimenterName;
        //参加者にメールを送る
        MailApp.sendEmail(ParticipantEmail, "【実験名・プロジェクト名】定員に達しました", text, {
            bcc: experimenterMailAddress
        });
        sheet.getRange(activeCellRow, 11).setValue(1);
        sheet.getRange(activeCellRow, 12).setValue('N/A');
...
function sendReminders(e) {
  try {
      ...
      var url = '参照するスプレッドシートのURL';
      var experimenterName = "実験者名";
      var experimenterMailAddress = "実験者のメールアドレス";
      var experimenterPhone = "研究室の番号や実験者の携帯番号";
      var experimentRoom = "実験場所";
              ...
              if ((reminder != "") && (reminder.getTime() <= time)) {
                  ...
                  if (week == 0 || week == 6) {
                      var text = ParticipantName + "  様\n\n実験者の" + experimenterName + 'です。明日参加していただく実験についての確認のメールをお送りしています。\n\n' +
                          '明日  ' + hour + ":" + min + "  から実験にご参加いただく予定となっております。\n" +
                          "場所は" + experimentRoom + "です。\n\nなお、明日は休日のため研究室のある建物の玄関の鍵がかかっており、外から入ることができません。実験者が実験開始5分前から玄関前で待機しておりますので、実験開始時間までにお越しください。\n\n" +
                          "ご不明な点などありましたら、" + experimenterMailAddress + "までご連絡ください。\n" +
                          "明日も、よろしくお願いいたします。\n\n" + experimenterName;
                  } else if (holiday > 0){
                      var text = ParticipantName + "  様\n\n実験者の" + experimenterName + 'です。明日参加していただく実験についての確認のメールをお送りしています。\n\n' +
                  ...
                  MailApp.sendEmail(ParticipantEmail, "【実験名・プロジェクト名】明日実施の実験のリマインダー", text, {
                      bcc: experimenterMailAddress
                  });
